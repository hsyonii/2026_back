...
public class Practice13 {
    public static void main(String[] args) {
        ...
        // 선생님 코드
        // 인터페이스를 쓰는 이유? 변수의 재사용(다형성)
        // 같은 패키지(폴더) 내 클래스명 중복 XX
        Soundable sound = new Cat2();
        sound.makeSound();
        sound = new Dog();
        sound.makeSound();
...
interface Soundable{
    //추상메소드
    void makeSound(); //public abstract 생략
}
//구현클래스
class Cat2 implements Soundable{
    @Override
    //void makeSound() -> (인터페이스 내 추상메소드 아닌)일반클래스 내 메소드 접근제한자 생략시 default로 설정
