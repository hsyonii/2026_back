...
public class Exam1 {
    public static void main(String[] args) {
        ...
        for(int i = 0; i<=fruits.size()-1; i++){//배열.size() : 배열의 크기 반환
            System.out.printf("index %d : %s",i,fruits.get(i)); //배열.get(인덱스) : 해당인덱스의 값 반환
        }
        ...
        alphabets.add("A"); alphabets.add("B"); alphabets.add("C"); alphabets.add("D"); alphabets.add("E");
        alphabets.remove(2);
        ...
        list5.add(1,"중간삽입"); // 인덱스 지정하여 요소 중간삽입 가능

        //문제6)
        //ArrayList<요소명 대신 클래스도 가능> 변수명 = new ArratList<>();
        ArrayList<Book> bookList = new ArrayList<>();
        bookList.add(new Book("책이름1","저자1"));
        bookList.add(new Book("책이름2","저자2"));
        bookList.add(new Book("책이름3","저자3"));
        //for(요소타입or데이터 변수명(임의) : 배열이름)
        //bookList로 부터 꺼낸 데이터를 Book타입의 변수 book에저장
