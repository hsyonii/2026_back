...
-- **테이블A inner join 테이블B on 조인조건
select * from table t1 Inner Join table2 t2 ON t1.num_pk = t2.num_fk;

-- 테이블A join 테이블B on 조인조건
SELECT * from  table1 t1 JOIN table2 t2 ON t1.num_pk = t2.num_fk;

-- 테이블A NATURAL JOIN 테이블B *전제조건 : PK와 FK 필드명 같은 경우
SELECT * FROM table1 t1 NATURAL JOIN table2 t2;

-- 테이블A JOIN 테이블B USING (필드명) *전제조건: pK와 FK 필드명 같은경우
SELECT * FROM table1 t1 join table2 t2 USING (num);

# OUTER JOIN
-- 테이블A LEFT OUTER JOIN 테이블B ON, 왼쪽 테이블에 모든 레코드와 오른쪽 테이블의 교집합되는 조회
SELECT * FROM table1 t1 LEFT OUTER JOIN table2 t2 ON t1.num_pk = t2.num_fk;
-- 테이블A RIGHT OUTER JOIN 테이블B ON, 오른쪽 테이블에 모든레코드와 왼쪽 테이블에 교집합 되는 조회
...
SELECT * FROM table1 t1 RIGHT JOIN tabel2 t2 ON t1.num_pk = t2.num_fk;
...
SELECT num_pk FROM table1 t1 RIGHT JOIN table2 t2 ON t1.num_pk = t2.num_fk WHERE num_fk IS NULL;
